...
export default {
  ...
  effects: {
    *getCustomerSettingPage(_, { call, put }) {
      const response = yield call(getCustomerSettingPage);
      //   if (!response) {
  ...
  subscriptions: {
    setup({ history, dispatch }) {
      ...
      return history.listen(({ pathname, search }) => {
        if (pathname === '/setting/customerSetting') {
          dispatch({
            type: 'customerSetting/getCustomerSettingPage',
          });
        }
