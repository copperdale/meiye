  ...
  effects: {
    *newOrUpdateEmployee(_, { call, put, select }) {
      try {
        ...
        if (id) {
          params.id = id;
          response = yield call(updateNewEmployee, params);
        }  else {
          params.enabledFlag = 1;
          params.sourceFlag = 1;
          response = yield call(addNewEmployee, params);
        }
        //   if (!response) {
        ...
        yield put(routerRedux.push('/employee'));
      } catch(e) {
        console.log(e);
      };
    },
    ...
    *getCurrentEmployee({ payload: { id } }, { call, put, select }) {
      ...
      const currenctEmployee = newFormFieldsKeys.reduce((result, item) => {
        ...
        result.passwordNum = '';
        if (dateKeys.indexOf(item) > -1) {
          // console.log(new Date(response.data[item]))
          result[item] = { value: moment(Number(response.data[item])) }
        }
        return result;
      }, {});
      yield put({
