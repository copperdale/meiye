...
import {
  getCommercialSettingPage,
} from '../services/loadHtml';

export default {
  namespace: 'commercialSetting',
  ...
  effects: {
    *getCommercialSettingPage(_, { call, put }) {
      const response = yield call(getCommercialSettingPage);
      //   if (!response) {
  ...
  subscriptions: {
    setup({ history, dispatch }) {
      ...
      return history.listen(({ pathname, search }) => {
        if (pathname === '/setting/commercialSetting') {
          dispatch({
            type: 'commercialSetting/getCommercialSettingPage',
          });
        }
